...
const page = () => {
  ...
  const handleSubmit = async() => {
    setLoading(true)
    try {
      const resp = await apiService.post('/api/Auth/ForgottenPassword', formInputs)
      console.log(resp)
...
<path d="M1 1L162 0.999986" stroke="#D6D6D6" stroke-linecap="round"/>
</svg> 
...
<path d="M1 1L162 0.999986" stroke="#D6D6D6" stroke-linecap="round"/>
        ...
        <form className=" flex flex-col w-full text-[#8f8f8f] gap-9">
            ...
            <Mail  className=" w-4 h-4 text-[#8f8f8f]" />
                <input onChange={handleChange} type="text" className=" border-none outline-none shadow-none w-full h-full" placeholder="Enter your email" />
            </div>
            ...
            <LockKeyhole className=" w-4 h-4 text-[#8f8f8f]" />
                <input onChange={handleChange} type="text" className=" border-none w-full shadow-none outline-none h-full" placeholder="Enter your password" />
            </div>
            <Link href={'/forgot-password'} className=" self-end text-[#0030AD] text-xs font-bold">Forgot password</Link>
            <button className=" bg-[#0030AD] rounded-[8px] py-3 font-bold text-white">Sign in</button>
           
        </form>
        <div className="flex">
            <p className=" text-[#8F8F8F] text-sm">Don't have an account?</p><Link href={'/signup'} className="text-[#0030AD] font-bold text-sm">Sign up</Link>
