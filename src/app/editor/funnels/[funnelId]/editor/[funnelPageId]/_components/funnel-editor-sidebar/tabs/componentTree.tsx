      ...
      <div>
        {content.map((item) => {
          if (Array.isArray(item.content)) {
            return (
              <div key={item.id}>
                <AccordionItem value={item.id}>
                  <AccordionTrigger onClick={(e) => handleOnClickBody(e, item)}>
                    {" "}
                    <span>{item.name}</span>{" "}
                  </AccordionTrigger>
            ...
            return (
              <div
                onClick={(e) => handleOnClickBody(e, item)}
        ...
        <div key={elem.id}>
          <Accordion type="multiple">
            <AccordionItem value={elem.id}>
              <AccordionTrigger onClick={(e) => handleOnClickBody(e, elem)}>
                {elem.name}
              </AccordionTrigger>
              <AccordionContent style={{ marginLeft: "20px" }}>
                {Array.isArray(elem.content) ? (
                  ...
                  <div
                    onClick={(e) => handleOnClickBody(e, elem)}
                    className=" cursor-pointer"
                  >
                    {elem.name}
                  </div>
